...
bool Text::setFont(char* fontPath, int size)
{
	...
	for (GLubyte c = 0; c < 128; c++)
	{
		...
		if ((*ftFace)->glyph->metrics.height >> 6 > face->height);
		{face->height = (*ftFace)->glyph->metrics.height >> 6; }
		if ((*ftFace)->glyph->metrics.vertBearingY >> 6 > face->bearingY);
		{face->bearingY = (*ftFace)->glyph->metrics.vertBearingY >> 6; }
	}
	face->height = face->height + face->bearingY;
	std::cout << "\nHighest height: " << face->height;
	std::cout << "\nHighest vert bearing: " << face->bearingY;
...
void Text::render()
{
	...
	{//new line
		x = beginX;
		y -= face->height + lineSpacing*scale;
	}
